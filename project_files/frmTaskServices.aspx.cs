...
namespace WebApplication2
{
	...
	/// </summary>
	public partial class frmTaskServices: System.Web.UI.Page
	{
		...
		private void loadData()
		{
			...
			cmd.CommandText="eps_RetrieveTaskServices";		
		...
		private void addTaskServices()
		{
			SqlCommand cmd=new SqlCommand();
			cmd.CommandType=CommandType.StoredProcedure;
			cmd.CommandText="eps_UpdateTaskServicesfromProfiles";//"eps_RetrieveProfileSPStaff";
			cmd.Connection=this.epsDbConn;
			cmd.Parameters.Add ("@TaskId",SqlDbType.Int);
			cmd.Parameters["@TaskId"].Value=Session["TaskId"].ToString();			
			cmd.Connection.Open();
		...
		private void itemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{			
				...
				Session["CResAll"]="frmTaskServices";
			...
			else if (e.CommandName == "Details")
			{
				...
				Session["TaskResId"]=e.Item.Cells[0].Text;
				Session["ResTypeName"]=e.Item.Cells[3].Text;
				Session["CallerDisInv"]="frmTaskServices";
				Response.Redirect (strURL + "frmDisplayInventory.aspx?"
					+ "&BackupId=" + e.Item.Cells[2].Text
					+ "&Type=" + Type);
			}
			else if (e.CommandName == "ServiceProvider")
			{
				Session["TaskSerId"]=e.Item.Cells[0].Text;
				Session["ServicesId"]=e.Item.Cells[1].Text;
				Session["ServiceName"]=e.Item.Cells[3].Text;
				Session["CSerProviders"]="frmTaskServices";
		...
		private void Exit()
		{
			Response.Redirect (strURL + Session["CallerTaskServices"].ToString() + ".aspx?");
		}
		...
		protected void btnAddTemp_Click(object sender, System.EventArgs e)
		{
			...
			Session["CSPResTypes"]="frmTaskServices";
