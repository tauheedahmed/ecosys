...
namespace WebApplication2
{
	...
	/// </summary>
	public partial class frmProjPeople: System.Web.UI.Page
	{
		...
		private void loadData()
		{
			...
			cmd.CommandText="wms_RetrieveOLPPPeople";		
			...
			if (ds.Tables["RoleStaff"].Rows.Count == 0)
			{
				...
				lblContents1.Text="There are no staff currently assigned to "
					+ Session["ProjTypeNameS"].ToString()
					+ " '" + Session["ProjName"].ToString()
					+ "'. Click on 'Add' to add staff.";
			}
			else
			{
				lblContents1.Text="Location: " 
					+ Session["LocationName"].ToString()
					+ "Identify Staff to play the staff role(s) needed to carry out process '"
		...
		{
			Session["CProjPeopleAdd"]="frmProjPeople";
			Response.Redirect (strURL + "frmProjPeopleAdd.aspx?");
		}
			...
			if (e.CommandName == "Remove")
				{
					SqlCommand cmd=new SqlCommand();
					cmd.CommandType=CommandType.StoredProcedure;
					cmd.CommandText="wms_DeleteOLPPPeople";
					cmd.Connection=this.epsDbConn;
		...
		private void Exit()
		{
			Response.Redirect (strURL + Session["COLPPP"].ToString() + ".aspx?");
		}
