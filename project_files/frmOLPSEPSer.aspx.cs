...
namespace WebApplication2
{
	...
	/// </summary>
	public partial class frmOLPSEPSer: System.Web.UI.Page
	{
		...
		private void loadData()
		{
			...
			cmd.CommandText="wms_RetrievePSEPSer";		
			cmd.Parameters.Add ("@PSEPID",SqlDbType.Int);
			cmd.Parameters["@PSEPID"].Value=Session["PSEPID"].ToString();	
			DataSet ds=new DataSet();
			SqlDataAdapter da=new SqlDataAdapter(cmd);
			da.Fill(ds,"ProcSer");
			if (ds.Tables["ProcSer"].Rows.Count == 0)
			{
				lblContents1.Text="Sorry, no services have been created for this procedure.  Contact the system administrator.";
			}
			Session["ds"] = ds;
		...
		*/
		...
		private void Exit()
		{
			Response.Redirect (strURL + Session["CPSer"].ToString() + ".aspx?");
		}
		...
		{
			if (e.CommandName == "Procurements")
			{
				Session["CPSP"]="frmOLPSEPSer";
				Session["PSEPSerId"]=e.Item.Cells[0].Text;
				Session["ServiceTypesId"]=e.Item.Cells[1].Text;
				//Response.Redirect (strURL + "frmStepSContracts.aspx?");
				Response.Redirect (strURL + "frmPSProcure.aspx?");
				/*Response.Redirect (strURL + ".aspx?");*/
			}
